...
/**
 * Classe responsable de la création de la fenêtre permettant de choisir ses bonus de jeu.
 * @author Tristan
 *
 */
public class BonusFrame implements ActionListener {
	 ...
	 * @param UUID String contenant l'identifiant universel de l'utilisateur.
	 */
	...
	public void initialize() {
		...
		// Ajout d'un ItemListener sur le toggle bouton pour afficher ou cacher le texte explicatif du bonus
		...
		// Thread permettant de regarder si des bonus existent en provenance des 3 autres jeux et de rafraîchir l'affichage en conséquence
		Thread t = new Thread() {
			public void run() {
				while(run) {
					// On récupère la liste des bonus disponibles pour l'utilisateur
					...
					for (Bonus bonus : listeBonus) {
						...
						if(bonus.getPossedeBonus()) {
							// Suivant le nom du jeu, on autorise le clique sur le bouton correspondant
						...
						// S'il n'y a pas de bonus pour le jeu
							...
							// Suivant le nom du jeu, on interdit le clic sur le bouton correspondant, on cache l'explication du bonus et on désactive les booléens des bonus
							switch(bonus.getNomJeu()) {
							case "howob" : 
								bonusHowob = false;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() instanceof JButton) {
			...
			if(btn.getActionCommand().equals("valider")) {
				// On lance la fenêtre principale avec l'identifiant universel du joueur et les bonus qu'il a choisit d'utiliser, on coupe le Thread et on ferme la fenêtre
